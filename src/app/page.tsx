    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8">
          {/* Tambahkan text-gray-800 ke container utama untuk pewarnaan teks otomatis */}
          <div className="text-gray-800">
            <h1 className="text-3xl font-bold text-center text-gray-800 mb-8">Kalkulator Deret Aritmatika</h1>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div>
                <label className="block text-sm font-medium text-gray-800 mb-1">Suku Awal (a)</label>
                {/* Tambahkan text-gray-800 ke input agar teksnya hitam saat diketik */}
                <input
                  type="number"
                  value={a}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-800 mb-1">Beda (b)</label>
                <input
                  type="number"
                  value={b}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-800 mb-1">Suku ke-n / Jumlah Suku (n)</label>
                <input
                  type="number"
                  value={n}
              ...
              <div className="mt-6 bg-gray-50 p-4 rounded-lg max-h-60 overflow-y-auto">
                ...
                {history.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {history.map((item) => (
                      <li key={item.id} className="text-sm bg-white p-3 rounded border border-gray-200 text-gray-800">
                        <div className="font-medium">{item.label}</div>
                        <div>{item.formula} = {item.result.toFixed(2)}</div>
                      </li>
                    ))}
                  </ul>
                )}
      ...
      {showModal && result !== null && (
        <div className="fixed inset-0 bg-transparent flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md text-gray-800">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Hasil Perhitungan</h2>
            <p className="mb-2"><span className="font-medium">Rumus:</span> {formulaUsed}</p>
            <p className="mb-2"><span className="font-medium">Perhitungan:</span> {label}</p>
